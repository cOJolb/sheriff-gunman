    ...
    public partial class MAnimal
    {
       ...
       public void OnEnable()
        ...
        internal void MovementSystem(float DeltaTime)
        {
            ...
            var LerpUpDown = DeltaTime * PitchLerpOn.Value;
            ...
            VerticalSmooth = LerpVertical > 0 ? 
                Mathf.Lerp(VerticalSmooth, MovementAxis.z * maxspeedV, LerpVertical) : 
                MovementAxis.z * maxspeedV;           //smoothly transitions bettwen Speeds
            HorizontalSmooth = LerpTurn > 0 ?
             
                Mathf.Lerp(HorizontalSmooth, MovementAxis.x * maxspeedH, LerpTurn) :
        ...
        protected void TryActivateState()
        {
            ...
            foreach (var trySt in states)
            {
                ...
                {
                    if (trySt.Active && !trySt.OnEnterCoolDown && !trySt.IsSleepFromState && !trySt.IsSleepFromMode && !trySt.OnQueue && trySt.TryActivate())
                    {
                        trySt.Activate();
                        break;
                    }
        ...
        {
            PitchDirection = Vector3.Lerp(PitchDirection, Move_Direction, DeltaTime * PitchLerpOn * 2);
        } 
        ...
        void OnAnimatorMove()
        {
            ...
            if (!FreeMovement && Rotator)
            {
                ...
                {
                    float limit = 0.005f;
                    var lerp = DeltaTime * (PitchLerpOff);
       ...
       // float RMWeight;
        ...
        public virtual void FreeMovementRotator(float Ylimit, float bank)
        {
            ...
            PitchAngle = Mathf.Lerp(PitchAngle, NewAngle * SmoothZYInput, DeltaTime * PitchLerpOn);
            Bank = Mathf.Lerp(Bank, -bank * Mathf.Clamp(HorizontalSmooth, -1, 1), DeltaTime * BankLerp);
